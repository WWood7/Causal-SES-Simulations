#!/bin/bash
# Causal Effect Size Simulation Script - RCT with pi!=0.5
# Usage: chmod u+x run_simulation_rct_pi.sh
# ./run_simulation_rct_pi.sh cohensd_rct_pi.r es_rct_pi

analysis=$2      # change for every analysis you run (2nd arg)
maildom='@emory.edu'   # your email domain (for receiving error messages)
myscratch="/home/wwu227/CSES_results/scratch_rct_pi"  # location of your persistent scratch dir
resultdir="/home/wwu227/CSES_results/scratch_rct_pi/out"  # This is a folder in permanent storage
...
for i in $(seq 1 ${loops}); do
	echo "#!/bin/bash" > cses_cohensd_pi$i.sh
	echo "#SBATCH --array=1-$max_jobs" >> cses_cohensd_pi$i.sh
	echo "#SBATCH --partition=day-long-cpu" >> cses_cohensd_pi$i.sh
	echo "#SBATCH --error=${myscratch}/err/${analysis}$i.err" >> cses_cohensd_pi$i.sh
	echo "#SBATCH --output=${myscratch}/out/${analysis}$i.out" >> cses_cohensd_pi$i.sh
	echo "#SBATCH --job-name=${analysis}_batch$i" >> cses_cohensd_pi$i.sh

	echo "source ~/miniconda3/etc/profile.d/conda.sh" >> cses_cohensd_pi$i.sh
	echo "conda activate r_fresh" >> cses_cohensd_pi$i.sh
	echo "Rscript ${script} $i" >> cses_cohensd_pi$i.sh

	# Submit job and capture job ID
	job_output=$(sbatch cses_cohensd_pi$i.sh)
